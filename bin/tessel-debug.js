#!/usr/bin/env node
var os = require('os'), pjson = require('../package.json'), common = require('../src/common'), temp = require('temp'), mkdirp = require('mkdirp'), path = require('path'), request = require('request'), fs = require('fs'), colors = require('colors');
temp.track();
var hostname = 'http://tessel-debug.herokuapp.com';
common.basic();
// Command-line arguments
var argv = require('nomnom').script('tessel-debug').option('script', {
    position: 1,
    full: 'script.js',
...
function initDebug(serial, wifi, info, next) {
  postToWeb('/new', {
    ...
    if (!res || res == false)
      return console.log('Could not communicate with host debug server', res);
    // res = JSON.parse(res);
    next && next(res);
  });
}
...
function userScript(id, client, urls) {
  ...
  client.init(function () {
    ...
    client.run(argv.script, [
      'tessel',
      argv.script
    ].concat(argv.arguments || []), argv, function () {
      // stop on script exit
