...
exports.copyDirSyncRecursive = function(sourceDir, newDirLocation, opts) {
    ...
    for(var i = 0; i < files.length; i++) {
        ...
        if(currFile.isDirectory()) {
            ...
            do {
                var tmpCurrFile = fs.lstatSync(symlinkFull[0] == '/' ? symlinkFull : _path.join(sourceDir, symlinkFull));
            ...
            if (tmpCurrFile.isDirectory()) {
                exports.copyDirSyncRecursive(symlinkFull[0] == '/' ? symlinkFull : _path.join(sourceDir, symlinkFull), _path.join(newDirLocation, files[i]), opts);
            } else {
                /*  At this point, we've hit a file actually worth copying... so copy it on over. */
                fCopyFile(symlinkFull[0] == '/' ? symlinkFull : _path.join(sourceDir, symlinkFull), _path.join(newDirLocation, files[i]));
            }
        } else {
            /*  At this point, we've hit a file actually worth copying... so copy it on over. */
